...
<CSSModeVars
  ...
  filter={/^icon-/}
  render={variable => {
    return <span style={{color: `var(--color-${variable})`}}><StyledOcticon icon={OctofaceIcon} /> --color-{variable}</span>
  }}
/>
